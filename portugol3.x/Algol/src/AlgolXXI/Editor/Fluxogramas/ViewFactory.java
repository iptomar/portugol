 ...
 *         Foundation
 */
public class ViewFactory extends DefaultCellViewFactory {
    ...
    protected EdgeView createEdgeView(Object v) {
        try {
            DefaultEdge edge = (DefaultEdge) v;
            String viewClass = (String) edge.getAttributes().get(VIEW_CLASS_KEY);

            EdgeView view = (EdgeView) Thread.currentThread().getContextClassLoader().loadClass(viewClass).newInstance();
            view.setCell(v);
            return view;
        } catch (Exception ex) {
        }
